...
Plans a path with WheelchairPlanner, time-parameterizes it with
``time_parameterize`` (from robot.wheelchair_planner), and plots position,
velocity, and acceleration profiles for selected joints.

Usage::

    pixi run python robot/trajectory.py
    pixi run python robot/trajectory.py --group wheelchair_arm
"""
...
from wheelchair_planning.wheelchair import wheelchair_robot_config  # noqa: E402
...
def main(
    group: str = "wheelchair_whole_body",
    planner: str = "bitstar",
    ...
    from robot.wheelchair_planner import _active_indices
    ...
    plot_trajectory(
        ...
        times,
        traj,
        joint_names=list(wheelchair_robot_config.joint_names),
