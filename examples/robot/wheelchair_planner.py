...
FULL_DOF = len(wheelchair_robot_config.joint_names)
...
def _active_indices(group: str) -> np.ndarray:
    ...
    full_joint_names = wheelchair_robot_config.joint_names
...
class WheelchairPlanner:
    """Motion planner for the wheelchair + xArm7 robot.

    A MotionPlanner is created eagerly at construction and reused across all
    plan_to_joints calls — the C++ VAMP collision environment is never
    ...
    Example::
        ...
        goal  = start.copy()
        goal[3:10] = [0.3, -0.5, 0.3, 1.8, -0.2, -0.6, 0.5]
        path = planner.plan_to_joints("wheelchair_arm", start, goal)
    """
    ...
    def default_start() -> np.ndarray:
        """Return the home full-body configuration (10-DOF)."""
    ...
    def plan_to_joints(
        ...
        """Plan a collision-free path for *group* between two 10-DOF configs.

        Returns:
            - ``"not same"`` if any frozen joint differs between start and goal.
            - ``np.ndarray`` of shape ``(N, 10)`` on success.
    ...
    def time_parameterize(
        ...
        if v_max is None:
            if wheelchair_robot_config.max_velocity is None:
                raise ValueError(
                    "v_max not supplied and wheelchair_robot_config.max_velocity "
                    "is None — either pass a scalar v_max or populate the config."
                )
            max_velocity = np.asarray(
                wheelchair_robot_config.max_velocity, dtype=np.float64
            )
        else:
            max_velocity = np.full(DOF, v_max, dtype=np.float64)

        if a_max is None:
            if wheelchair_robot_config.max_acceleration is None:
                raise ValueError(
                    "a_max not supplied and wheelchair_robot_config."
                    "max_acceleration is None — either pass a scalar a_max "
                    "or populate the config."
                )
            max_acceleration = np.asarray(
                wheelchair_robot_config.max_acceleration, dtype=np.float64
            )
        else:
