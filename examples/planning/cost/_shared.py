...
:class:`~wheelchair_planning.planning.costs.Cost` — a scalar CasADi
expression that RRT*-family planners integrate along every motion —
rather than a hard :class:`Constraint`.  The per-state penalty is always
``ca.sumsqr(residual)`` of the same residual used in the constrained
demo, so the "manifold" geometry is identical; the planner just sees it
as a preference instead of a law.

We still reuse :meth:`SymbolicContext.project` for goal selection.
Projecting an otherwise-random seed onto ``residual = 0`` gives a goal
that sits exactly on the soft manifold, which makes the cost's shaping
effect visually obvious in the animation (the arm hugs the manifold
whenever it can).
"""
...
def find_goal(ctx, residual, start, planner, score, n: int = 400, seed: int = 0):
    ...
    valid config whose TCP position maximises ``score``.  Valid means
    inside the joint bounds AND collision-free under *planner*.
