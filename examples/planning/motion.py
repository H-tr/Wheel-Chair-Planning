...
Loads the bundled ``table.ply`` point cloud, places it in front of the
robot, and asks the 7-DOF xArm7 planner to weave a path from HOME to a
randomly sampled collision-free goal.  The C++ collision checker sees
every point as a small sphere (``point_radius``), so the planner has to
curve the arm around the table to reach the sampled goal.
...
def load_table(distance: float = 1.4, height: float = 0.9) -> np.ndarray:
    """Load ``table.ply`` and place it in front of the robot.

    The bundled scan lives a few metres away from the origin in its own
    coordinate frame, so we recentre it and push it forward along ``+x``.
    The defaults place it within the xArm7's workspace (the arm is mounted
    high on the wheelchair). Returns an ``(N, 3) float32`` cloud.
    """
...
def main(
    ...
    print(
        f"── motion planning: xArm7 around a table ──\n"
