...
:class:`wheelchair_planning.utils.video_recorder.VideoRecorder`.
...
from wheelchair_planning.types import (  # noqa: E402
    IKConfig,
    PinkIKConfig,
    PlannerConfig,
    SE3Pose,
...
def _make_env() -> PyBulletEnv:
    ...
    return PyBulletEnv(wheelchair_robot_config, visualize=False)
...
def record_subgroup_planning(out: Path) -> None:
    ...
    def base_with_stance(stance: dict[str, float]) -> np.ndarray:
        ...
        for name, val in stance.items():
            base[wheelchair_robot_config.joint_names.index(name)] = val
        return base

    # A curated set that reads well in a short clip: one per stance.
    combos = [
        ("wheelchair_left_arm", "high"),
        ("wheelchair_torso_left_arm", "mid"),
        ("wheelchair_dual_arm", "low"),
    ]
