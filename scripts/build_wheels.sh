...
#
# Builds wheelchair-vamp version-specific wheels for Python 3.10, 3.11, 3.12.
...
docker run --rm \
  ...
  bash -c '
    ...
    # ------------------------------------------------------------------
    # Step 3: Build wheelchair-vamp version-specific wheels
    # ------------------------------------------------------------------
    PYTHON_VERSIONS="cp312-cp312 cp311-cp311 cp310-cp310"

    for PYVER in $PYTHON_VERSIONS; do
      echo ""
      echo "==> Building wheelchair-vamp wheel for $PYVER..."
    ...
    # ------------------------------------------------------------------
    echo ""
    echo "==> Repairing wheelchair-vamp wheels..."
    for whl in /tmp/raw-wheels/wheelchair_vamp-*.whl; do
      echo ""
      echo "    --- $(basename $whl) ---"
    ...
    # ------------------------------------------------------------------
    # Step 5: Build wheelchair-planning wheel
    # (No KDL in this container → pytracik is skipped → pure Python wheel)
    # ------------------------------------------------------------------
    echo ""
    echo "==> Building wheelchair-planning wheel..."
