...
class MotionPlanner:
    ...
    reduced DOF space used by the planner and the full 10-DOF body
    configuration.
    """
    ...
    def __init__(
        ...
        # Frozen 10-DOF joint values for any joint not controlled by
        # this planner.  Defaults to HOME_JOINTS, but the caller can
        # pass any 10-DOF array — e.g. the live config from the env —
        ...
        full_names = wheelchair_robot_config.joint_names
    ...
    def subgroup_indices(self) -> np.ndarray | None:
        """Indices of this planner's joints in the full 10-DOF config."""
        return self._subgroup_indices

    @property
    def base_config(self) -> np.ndarray:
        """The 10-DOF stance frozen for joints outside this subgroup."""
    ...
    def _push_constraints(self, constraints) -> None:
        ...
        from wheelchair_planning.planning.constraints import Constraint

        for c in constraints:
            if not isinstance(c, Constraint):
                raise TypeError(
                    f"constraints must be Constraint instances from "
                    f"wheelchair_planning.planning.constraints; "
    ...
    def _push_costs(self, costs) -> None:
        ...
        from wheelchair_planning.planning.costs import Cost

        for c in costs:
            if not isinstance(c, Cost):
                raise TypeError(
                    f"costs must be Cost instances from "
                    f"wheelchair_planning.planning.costs; "
                    f"got {type(c).__name__}"
                )
            if c.ambient_dim != self._ndof:
                raise ValueError(
    ...
    def set_subgroup(
        ...
        Args:
            robot_name: Subgroup name from ``PLANNING_SUBGROUPS``, or
                ``"wheelchair"`` for the full 10-DOF body.
            base_config: 10-DOF frozen config for inactive joints.
    ...
    def extract_config(self, full_config: np.ndarray) -> np.ndarray:
        """Extract this planner's joints from a full 10-DOF configuration."""
    ...
    def embed_config(
        ...
        """Embed a subgroup config into a full 10-DOF configuration.

        ``base_config`` defaults to the planner's stored base — the same
        10-DOF values the C++ collision checker injects for inactive
    ...
    def embed_path(
        ...
        """Convert a subgroup path ``(N, sub_dof)`` to ``(N, 10)``.
...
def available_robots() -> list[str]:
    ...
    from wheelchair_planning.wheelchair import PLANNING_SUBGROUPS

    return ["wheelchair"] + sorted(PLANNING_SUBGROUPS.keys())


def create_planner(
    robot_name: str = "wheelchair",
    config: PlannerConfig | None = None,
    pointcloud: np.ndarray | None = None,
    base_config: np.ndarray | None = None,
    constraints: list | None = None,
    ...
    Args:
        ...
        base_config: 10-DOF values to inject for joints not controlled
            by this planner (i.e. the frozen joints of a subgroup).
            Defaults to ``HOME_JOINTS``.  Supply any 10-DOF array — for
            example the live configuration read from your env — to pin
            the rest of the body wherever it currently is.  Ignored for
            the full-body ``"wheelchair"`` planner.
        constraints: Optional list of
            :class:`~wheelchair_planning.planning.constraints.Constraint`
        ...
        costs: Optional list of
            :class:`~wheelchair_planning.planning.costs.Cost` instances
