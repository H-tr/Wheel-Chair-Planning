...
in :mod:`wheelchair_planning.planning.symbolic`).
"""
...
def _cache_root() -> Path:
    ...
    Honours ``WHEELCHAIR_CONSTRAINT_CACHE_DIR`` if set (useful for CI).
    Otherwise falls back to ``~/.cache/wheelchair_planning/constraints``.
    """
    override = os.environ.get("WHEELCHAIR_CONSTRAINT_CACHE_DIR")
    if override:
        return Path(override).expanduser().resolve()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return (base / "wheelchair_planning" / "constraints").resolve()
...
class Constraint:
    ...
    Constructing this class triggers (on cold cache):
        ...
        4. caching under ``~/.cache/wheelchair_planning/constraints/<sha>/``
    ...
    def __post_init__(self) -> None:
        ...
        if not so_path.exists():
            sys.stderr.write(f"[wheelchair] compiling constraint {sha[:8]}... ")
            sys.stderr.flush()
            t0 = time.perf_counter()
            with _cwd(cache_dir):
                f.generate("constraint.c")
            compiler = os.environ.get("WHEELCHAIR_CONSTRAINT_CC", "c++")
            subprocess.run(
                [
                    compiler,
                    "-O3",
