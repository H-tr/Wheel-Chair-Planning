...
:class:`wheelchair_planning.planning.symbolic.SymbolicContext`.  The
...
:class:`wheelchair_planning.planning.constraints.Constraint`: same
...
def _cache_root() -> Path:
    ...
    Honours ``WHEELCHAIR_COST_CACHE_DIR`` if set.  Otherwise falls back
    to ``~/.cache/wheelchair_planning/costs``.
    """
    override = os.environ.get("WHEELCHAIR_COST_CACHE_DIR")
    if override:
        return Path(override).expanduser().resolve()
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return (base / "wheelchair_planning" / "costs").resolve()
...
class Cost:
    ...
    vector.  Construction triggers (on cold cache):
        ...
        4. caching under ``~/.cache/wheelchair_planning/costs/<sha>/``
    ...
    def __post_init__(self) -> None:
        ...
        if not so_path.exists():
            sys.stderr.write(f"[wheelchair] compiling cost {sha[:8]}... ")
            sys.stderr.flush()
            t0 = time.perf_counter()
            with _cwd(cache_dir):
                f.generate("cost.c")
            compiler = os.environ.get("WHEELCHAIR_COST_CC", "c++")
            subprocess.run(
                [
                    compiler,
                    "-O3",
