...
The Wheelchair robot exposes 24 joints when you load the full URDF.
...
of indices into the 10-DOF body. The OMPL planner reasons in the
...
[`available_robots()`](../api/planning.md#wheelchair_planning.planning.motion_planner.available_robots):

| Alias | DOF | What it controls |
|---|---:|---|
| `wheelchair_base` | 3 | mobile base (x, y, yaw) |
| `wheelchair_height` | 3 | ankle + knee + waist pitch |
| `wheelchair_arm` | 7 | single arm, shoulder → wrist |
| `wheelchair_arm` | 7 | single arm, shoulder → wrist |
| `wheelchair_whole_body` | 14 | both arms |
| `wheelchair_torso_left_arm` | 9 | 2-DOF waist + left arm |
| `wheelchair_torso_right_arm` | 9 | 2-DOF waist + right arm |
| `wheelchair_whole_body` | 21 | whole body, base excluded |
| `wheelchair` | 24 | full body |

They exist as starting points. The list is a dict in
`wheelchair_planning.wheelchair.PLANNING_SUBGROUPS` — add your own entry
...
[`set_subgroup(active_indices, frozen_config)`](../api/planning.md#wheelchair_planning._ompl_vamp.OmplVampPlanner.set_subgroup),
which preserves the collision environment.

## When it matters

Planning in 10 DOF for a task that only needs an arm wastes almost
...
median. The same plan over the 10-DOF body runs 2–5× slower for no
