...
// Our wheelchair FK/collision header lives in-tree so the vamp submodule
// stays a pure header-only third-party dep. ``ext/ompl_vamp`` is on the
// include path, which makes this resolve to
// ``ext/ompl_vamp/robot/wheelchair.hh``.
#include <vector>

#include "robot/wheelchair.hh"

namespace wheelchair {

namespace ob = ompl::base;

using Robot = vamp::robots::Wheelchair;
