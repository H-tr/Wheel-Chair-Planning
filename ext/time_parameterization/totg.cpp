 ...
 * Date:   05/2012
 *
 * Standalone adaptation of the MoveIt 2 vendoring of Kunz & Stilman's TOTG
 * (moveit_core/trajectory_processing/time_optimal_trajectory_generation.cpp) —
 * ROS/MoveIt dependencies stripped so the core path- and time-parameterisation
 * routines depend only on Eigen and the C++17 standard library.  Diagnostic
 * messages that previously went through `rclcpp` are emitted to std::cerr.
 *
 * See LICENSE.TOTG alongside this file for the full BSD-style license text.
 */
...
namespace wheelchair::trajectory {
...
inline void warn(const char* msg) {
  std::cerr << "[wheelchair.trajectory] " << msg << std::endl;
}

}  // namespace

// -----------------------------------------------------------------------------
// Internal path segment implementations.  These deliberately mirror the
// upstream MoveIt layout and algebra so future upstream fixes port cleanly.
// -----------------------------------------------------------------------------
...
class CircularPathSegment : public PathSegment {
 public:
  CircularPathSegment(const Eigen::VectorXd& start,
    ...
    if ((intersection - start).norm() < 1e-6 ||
        (end - intersection).norm() < 1e-6) {
      length_ = 0.0;
      radius_ = 1.0;
    ...
    const double angle = std::acos(start_dot_end);
    ...
    distance = std::min(distance, max_deviation * std::sin(0.5 * angle) /
                                      (1.0 - std::cos(0.5 * angle)));
...
std::optional<Path> Path::create(const std::vector<Eigen::VectorXd>& waypoints,
                                 double max_deviation) {
  if (waypoints.size() < 2) {
    warn("Path::create: a path needs at least 2 waypoints.");
    return std::nullopt;
  }
  if (max_deviation <= 0.0) {
    warn("Path::create: max_deviation must be strictly greater than 0.0.");
    return std::nullopt;
  }

  Path path;
  auto it1 = waypoints.begin();
  auto it2 = std::next(it1);
  auto it3 = waypoints.end();
  Eigen::VectorXd start_config = *it1;
  while (it2 != waypoints.end()) {
    it3 = std::next(it2);
    if (it3 != waypoints.end()) {
      // Reject 180° reversals — not representable in the current blending.
      Eigen::VectorXd incoming = *it2 - *it1;
      Eigen::VectorXd outgoing = *it3 - *it2;
      double in_norm = incoming.norm();
      double out_norm = outgoing.norm();
      if (in_norm > std::numeric_limits<double>::epsilon() &&
          out_norm > std::numeric_limits<double>::epsilon()) {
        double cos_angle = incoming.dot(outgoing) / (in_norm * out_norm);
        constexpr double angle_tolerance = 1e-5;
        if (cos_angle <= -1.0 + angle_tolerance) {
          warn(
              "Path::create: path requires a 180-degree turn, which is "
              "unsupported.");
          return std::nullopt;
        }
      }
    }
    if (max_deviation > 0.0 && it3 != waypoints.end()) {
      auto* blend_segment = new CircularPathSegment(
          0.5 * (*it1 + *it2), *it2, 0.5 * (*it2 + *it3), max_deviation);
      Eigen::VectorXd end_config = blend_segment->getConfig(0.0);
      if ((end_config - start_config).norm() > 1e-6) {
        path.path_segments_.push_back(
      ...
      path.path_segments_.push_back(
          std::make_unique<LinearPathSegment>(start_config, *it2));
      start_config = *it2;
    }
    it1 = it2;
    ++it2;
  ...
  // Aggregate switching points and assign absolute arc-length positions.
  for (std::unique_ptr<PathSegment>& path_segment : path.path_segments_) {
    ...
    for (double local : local_switching_points) {
      path.switching_points_.emplace_back(path.length_ + local, false);
    }
    path.length_ += path_segment->getLength();
    while (!path.switching_points_.empty() &&
           path.switching_points_.back().first >= path.length_) {
      path.switching_points_.pop_back();
    }
    path.switching_points_.emplace_back(path.length_, true);
  }
  path.switching_points_.pop_back();
  return path;
}
...
Path::Path(const Path& other)
    : length_(other.length_), switching_points_(other.switching_points_) {
  for (const std::unique_ptr<PathSegment>& path_segment :
       other.path_segments_) {
...
Path& Path::operator=(const Path& other) {
  ...
  switching_points_ = other.switching_points_;
...
Eigen::VectorXd Path::getConfig(double s) const {
  return getPathSegment(s)->getConfig(s);
}

Eigen::VectorXd Path::getTangent(double s) const {
  return getPathSegment(s)->getTangent(s);
}

Eigen::VectorXd Path::getCurvature(double s) const {
  return getPathSegment(s)->getCurvature(s);
}
...
std::optional<Trajectory> Trajectory::create(
  ...
  while (output.valid_ &&
         !output.integrateForward(output.trajectory_, after_acceleration) &&
         output.valid_) {
    double before_acceleration;
    TrajectoryStep switching_point;
    if (output.getNextSwitchingPoint(output.trajectory_.back().path_pos_,
                                     switching_point, before_acceleration,
                                     after_acceleration)) {
      break;
    }
    output.integrateBackward(output.trajectory_, switching_point.path_pos_,
                             switching_point.path_vel_, before_acceleration);
  }

  if (!output.valid_) {
    warn(
        "Trajectory::create: invalid after integrateForward + "
        "integrateBackward pass.");
    return std::nullopt;
  }

  double before_acceleration =
      output.getMinMaxPathAcceleration(output.path_.getLength(), 0.0, false);
  output.integrateBackward(output.trajectory_, output.path_.getLength(), 0.0,
                           before_acceleration);

  if (!output.valid_) {
    warn("Trajectory::create: invalid after the final integrateBackward pass.");
    return std::nullopt;
  }

  // Calculate timing.
  auto previous = output.trajectory_.begin();
  auto it = previous;
  it->time_ = 0.0;
  ++it;
...
Trajectory::Trajectory(const Path& path, const Eigen::VectorXd& max_velocity,
    ...
    : path_(path),
      ...
      joint_num_(static_cast<std::size_t>(max_velocity.size())),
...
bool Trajectory::integrateForward(std::list<TrajectoryStep>& trajectory,
  ...
  while (true) {
    ...
    if (next_discontinuity != switching_points.end() &&
      ...
      // Avoid clustering near a discontinuity (see moveit/moveit#1665).
    ...
    if (path_pos > path_.getLength()) {
      ...
      valid_ = false;
      warn("integrateForward: negative path velocity encountered.");
      return true;
    }
    ...
    if (path_vel == 0 && acceleration == 0) {
      // position will never change if velocity and acceleration are both zero
      valid_ = false;
      warn(
          "integrateForward: zero acceleration and velocity — check that no "
          "joint's "
          "acceleration limit is zero.");
      return true;
    }

    if (path_vel > getAccelerationMaxPathVelocity(path_pos) ||
        path_vel > getVelocityMaxPathVelocity(path_pos)) {
      // Bisect to find a more accurate intersection with the max-velocity
      // curve.
...
void Trajectory::integrateBackward(std::list<TrajectoryStep>& start_trajectory,
  ...
  while (start1 != start_trajectory.begin() || path_pos >= 0.0) {
    if (start1->path_pos_ <= path_pos) {
      ...
      if (path_vel < 0.0) {
        valid_ = false;
        warn("integrateBackward: negative path velocity encountered.");
    ...
    // Check for intersection between the current start trajectory and backward
    // trajectory segments.
  ...
  valid_ = false;
  warn("integrateBackward: did not hit start trajectory.");
  end_trajectory_ = trajectory;
}
...
double Trajectory::getAccelerationMaxPathVelocity(double path_pos) const {
  ...
  for (std::size_t i = 0; i < joint_num_; ++i) {
    if (config_deriv[i] != 0.0) {
      for (std::size_t j = i + 1; j < joint_num_; ++j) {
      ...
      max_path_velocity = std::min(
          max_path_velocity,
          std::sqrt(max_acceleration_[i] / std::abs(config_deriv2[i])));
    }
  }
...
double Trajectory::getVelocityMaxPathVelocityDeriv(double path_pos) const {
  const Eigen::VectorXd tangent = path_.getTangent(path_pos);
  double max_path_velocity = std::numeric_limits<double>::max();
  std::size_t active_constraint = 0;
  for (std::size_t i = 0; i < joint_num_; ++i) {
